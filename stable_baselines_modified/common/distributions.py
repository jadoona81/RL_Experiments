...
class CategoricalProbabilityDistribution(ProbabilityDistribution):
    ...
    def sample(self):
        ...
        uniform = tf.compat.v1.random_uniform(tf.shape(self.logits), dtype=self.logits.dtype)
        return tf.compat.v1.argmax(self.logits - tf.compat.v1.log(-tf.compat.v1.log(uniform)), axis=-1)
