...
def build_act(q_func, ob_space, ac_space, stochastic_ph, update_eps_ph, sess):
 ...
 #tf.get_variable("eps", (), initializer=tf.constant_initializer(0))

    policy = q_func(sess, ob_space, ac_space, 1, 1, None)
    obs_phs = (policy.obs_ph, policy.processed_obs)
    print('**************************************************')
    print(policy.q_values)
    deterministic_actions = tf.argmax(policy.q_values, axis=1) #tf.argmax(policy.q_values, axis=1)
...
#tf.variable_scope("perturbed_model", reuse=False):
...
def build_train(q_func, ob_space, ac_space, optimizer, sess, grad_norm_clipping=None,
    ...
    """
    n_actions = ac_space.nvec if isinstance(ac_space, MultiDiscrete) else ac_space.n
    with tf.compat.v1.variable_scope("input", reuse=reuse): #tf.variable_scope("input", reuse=reuse):
        stochastic_ph =  tf.compat.v1.placeholder(tf.bool, (), name="stochastic") #tf.variable_scope("input", reuse=reuse):
#tf.placeholder(tf.bool, (), name="stochastic")
        update_eps_ph =  tf.compat.v1.placeholder(tf.float32, (), name="update_eps") #tf.placeholder(tf.float32, (), name="update_eps")

    with tf.compat.v1.variable_scope(scope, reuse=reuse): #tf.variable_scope(scope, reuse=reuse):
        if param_noise:
